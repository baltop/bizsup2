#!/usr/bin/env python3
"""
Enhanced YIPA (영월산업진흥원) Scraper v2.0
Site: https://mybiz.yipa.or.kr/yipa/bbs_list.do?code=sub01b&keyvalue=sub01
Uses Playwright for browser automation to handle complex session-based access
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('yipa_scraper_v2.log', encoding='utf-8'),
...
class YIPAScraperV2:
    ...
    def parse_post_list_with_playwright(self, list_url):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # 게시글 링크들 추출
                post_links = page.query_selector_all('a[href*="bbs_view"]')
            ...
            try:
                ...
                post_info = page.evaluate("""
                    () => {
                        ...
                        if (rows.length > 0) {
                            // 첫 번째 행: 제목
                            const titleCell = rows[0].querySelector('td');
                            if (titleCell) {
                                info.title = titleCell.textContent.trim();
                            }
                            
                            // 두 번째 행: 작성자, 날짜 등
                            if (rows.length > 1) {
                                const infoCell = rows[1].querySelector('td');
                                if (infoCell) {
                                    info.authorDate = infoCell.textContent.trim();
                                }
                            }
                            
                            // 세 번째 행: 내용
                            if (rows.length > 2) {
                                const contentCell = rows[2].querySelector('td');
                                if (contentCell) {
                                    info.content = contentCell.innerHTML;
                                }
                            }
                        }
                ...
                # 내용을 마크다운으로 변환
                content_html = post_info.get('content', '')
                content_markdown = self.h2t.handle(content_html) if content_html else "내용이 없습니다."
                
                # 작성자와 날짜 파싱
                author_date = post_info.get('authorDate', '')
                author = ""
                date = ""
                
                if author_date:
                    # "경영혁신팀    2023년 11월 06일 17시 18분" 형태 파싱
                    parts = author_date.split()
                    if len(parts) >= 2:
                        author = parts[0]
                        date = ' '.join(parts[1:])
    ...
    def save_processed_titles(self):
        ...
        try:
            json_file = self.output_dir / f"processed_titles_enhanced{self.site_code}_v2.json"
            with open(json_file, 'w', encoding='utf-8') as f:
                json.dump(self.processed_titles, f, ensure_ascii=False, indent=2)
            logger.info(f"처리된 제목 정보 저장 완료: {json_file}")
        except Exception as e:
            logger.error(f"processed_titles_enhanced{self.site_code}_v2.json 저장 실패: {e}")
