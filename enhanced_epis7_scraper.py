...
class EnhancedEpis7Scraper(EnhancedBaseScraper):
    """EPIS 입찰/공모 게시판 스크래퍼"""
    
    def __init__(self):
        super().__init__()
        self.base_url = "https://www.epis.or.kr"
        self.list_url = "https://www.epis.or.kr/home/kor/M653084945/structure/board/index.do"
        self.site_code = "epis7"
        ...
        for row_index, row in enumerate(rows):
            try:
                ...
                if not href or href == 'javascript:void(0);':
                    ...
                    if onclick:
                        # fn_edit 함수 호출에서 idx 추출 (단일 파라미터)
                        match = re.search(r'fn_edit\(["\']([^"\']+)["\']', onclick)
                        if match:
                            idx = match.group(1)  # 실제 idx 값
                            detail_url = f"https://www.epis.or.kr/home/kor/M653084945/structure/board/edit.do?idx={idx}&searchValue2=&searchValue3=&searchKeyword=&pageIndex={self.current_page_num}"
                        else:
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('epis7_scraper.log', encoding='utf-8'),
            logging.StreamHandler()
        ]
    )
    
    # 스크래퍼 실행
    scraper = EnhancedEpis7Scraper()
    ...
    try:
        ...
        # 3페이지 수집
        logger.info("="*60)
        logger.info(f"🚀 EPIS 입찰/공모 게시판 스크래핑 시작 (새로운 수집)")
