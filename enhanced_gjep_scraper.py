#!/usr/bin/env python3
"""
Enhanced GJEP (경기도일자리종합센터) Scraper
...
class GJEPScraper:
    ...
    def download_file(self, file_url, attachments_dir, original_filename):
        ...
        try:
            ...
            if not file_url.startswith('http'):
                file_url = urljoin('https://www.gjep.or.kr/', file_url)
    ...
    def extract_notice_list(self, soup):
        ...
        for row in notice_rows:
            try:
                ...
                # Extract notice number/type (first cell might be empty)
                number_cell = cells[0]
                number = number_cell.get_text(strip=True)
                
                # Extract title and link from title cell
                title_cell = cells[1]
                title_links = title_cell.find_all('a', href=True)
                
                # Find the actual notice link (not the category link)
                title_link = None
                ...
                notice_id = ""
                if 'dk_id=' in detail_url:
                    id_match = re.search(r'dk_id=(\d+)', detail_url)
                    if id_match:
                        notice_id = id_match.group(1)
                
                # Extract category from the category link (first link in cell)
                category = ""
                category_link = title_cell.find('a', href=True)
                if category_link and 'sca=' in category_link.get('href', ''):
                    category = category_link.get_text(strip=True)
                
                # Extract author (usually cells[2])
                author_cell = cells[2]
                ...
                # Extract views (last cell)
                views_cell = cells[4]
                views = views_cell.get_text(strip=True)
                
                # Check for attachments - look for attachment icon anywhere in the row
                has_attachment = False
                for cell in cells:
                    if cell.find('img', alt='첨부파일 아이콘'):
                        has_attachment = True
                        break
                
                # If detail_url is relative, make it absolute
                if detail_url and not detail_url.startswith('http'):
                    detail_url = urljoin('https://www.gjep.or.kr/cms/bbs/', detail_url)
    ...
    def scrape_notice_detail(self, notice_data):
        ...
        try:
            ...
            # Extract content from viewimg cell
            content_html = ""
            
            # Try different possible content containers
            content_selectors = [
                'td.viewimg',              # Main content container
                'div#writeContents',       # Write contents
                '.view_content',           # Generic view content
            ...
            # Look for attachment download links using JavaScript pattern
            # javascript:file_download('./download.php?dk_cms=comm_01&dk_id=XXX&no=Y&page=Z', 'filename');
            import html
            page_html = str(soup)
            ...
            for download_url, filename in download_patterns:
                if download_url and filename:
                    # Clean up the download URL and decode HTML entities
                    ...
                    downloaded_file, file_size = self.download_file(download_url, attachments_dir, filename)
                    if downloaded_file:
                        downloaded_files.append((downloaded_file, file_size))
            
            # Also look for direct attachment links
            attachment_links = soup.find_all('a', href=re.compile(r'download\.php'))
            for link in attachment_links:
                ...
                if href and not href.startswith('javascript:'):
                    ...
                    if not filename:
                        filename = f"attachment_{len(downloaded_files) + 1}"
                    
                    if not href.startswith('http'):
                        href = urljoin('https://www.gjep.or.kr/cms/bbs/', href)
                    
                    downloaded_file, file_size = self.download_file(href, attachments_dir, filename)
                    if downloaded_file:
                        downloaded_files.append((downloaded_file, file_size))
    ...
    def create_markdown_content(self, notice_data, content_markdown, detail_url):
        ...
        content += f"**분류:** {notice_data['category']}\n\n"
...
def main():
    # Configuration
    base_url = "https://www.gjep.or.kr/cms/bbs/cms.php?dk_cms=comm_01"
    site_code = "gjep"
    max_pages = 3
    
    # Initialize scraper
    scraper = GJEPScraper(base_url, site_code)
