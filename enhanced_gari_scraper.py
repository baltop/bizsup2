...
class GARIScraper:
    ...
    def download_file(self, file_url, attachments_dir, original_filename):
        ...
        try:
            ...
            if not file_url.startswith('http'):
                file_url = urljoin('https://www.gari.or.kr/', file_url)
    ...
    def extract_notice_list(self, soup):
        ...
        notice_table = soup.find('table', class_='basicList')
        if not notice_table:
        ...
        if not tbody:
            self.logger.warning("No tbody found in notice table")
            return notices
        ...
        for row in notice_rows:
            try:
                ...
                # Extract notice ID from href JavaScript
                href = title_link.get('href', '')
                onclick = title_link.get('onclick', '')
                notice_id = ""
                self.logger.debug(f"Href attribute: {href}")
                self.logger.debug(f"Onclick attribute: {onclick}")
                
                # Try to extract from href first (javascript:viewData('ID'))
                if 'viewData' in href:
                    id_match = re.search(r"viewData\('(\d+)'\)", href)
                    if id_match:
                        notice_id = id_match.group(1)
                        self.logger.debug(f"Extracted notice ID from href: {notice_id}")
                
                # If not found in href, try onclick
                if not notice_id and 'viewData' in onclick:
                    id_match = re.search(r"viewData\('(\d+)'\)", onclick)
                    if id_match:
                        notice_id = id_match.group(1)
                        self.logger.debug(f"Extracted notice ID from onclick: {notice_id}")
                
                # If no notice ID found, use row number as fallback
                if not notice_id:
                    notice_id = number
                    self.logger.debug(f"Using fallback notice ID: {notice_id}")
                ...
                detail_url = f"https://www.gari.or.kr/boardView.do?boardId=40&dataNo={notice_id}&nowPage=1&sub=10_01"
    ...
    def scrape_notice_detail(self, notice_data):
        ...
        try:
            ...
            content_selectors = [
                '.basicView .conText',        # Main content container
                '.basicView .conField',       # Content field
                '.basicView',                 # Entire view container
                '.conText',                   # Generic content text
            ...
            title_field = soup.find('div', class_='titleField')
            if title_field:
                # Extract views, registration date, and author
                view_span = title_field.find('span')
                if view_span:
                    views_text = view_span.get_text(strip=True)
                    views_match = re.search(r'(\d+)', views_text)
                    if views_match:
                        notice_data['views'] = views_match.group(1)
                
                # Extract registration date and author from list items
                list_items = title_field.find_all('li')
                for item in list_items:
                    text = item.get_text(strip=True)
                    if '등록일' in text:
                        date_match = re.search(r'(\d{4}년\s+\d{1,2}월\s+\d{1,2}일)', text)
                        if date_match:
                            notice_data['reg_date'] = date_match.group(1)
                    elif '작성자' in text:
                        author_match = re.search(r'작성자\s*:\s*(.+)', text)
                        if author_match:
                            notice_data['author'] = author_match.group(1)
            ...
            attachment_links = soup.find_all('a', href=re.compile(r'fileDownload\.do'))
    ...
    def create_markdown_content(self, notice_data, content_markdown, detail_url):
        ...
        content += f"**기간:** {notice_data['date_range']}\n\n"
        content += f"**부서:** {notice_data['department']}\n\n"
        content += f"**상태:** {notice_data['status']}\n\n"
        
        if 'views' in notice_data:
            content += f"**조회수:** {notice_data['views']}\n\n"
        
        if 'reg_date' in notice_data:
    ...
    def scrape_pages(self, max_pages=3):
        ...
        for page_num in range(1, max_pages + 1):
            ...
            if page_num == 1:
                ...
                page_url = f"{self.base_url}{separator}nowPage={page_num}"
...
def main():
    # Configuration
    base_url = "https://www.gari.or.kr/boardList.do?boardId=40&sub=10_01"
    site_code = "gari"
    max_pages = 3
    
    # Initialize scraper
    scraper = GARIScraper(base_url, site_code)
