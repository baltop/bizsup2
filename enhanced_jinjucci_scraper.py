# -*- coding: utf-8 -*-
"""
Jinjucci(진주상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedJinjucciScraper(StandardTableScraper):
    ...
    def __init__(self):
        ...
        # Jinjucci 특화 설정 - 타임아웃 대기시간 대폭 증가
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for row in data_rows:
            try:
                ...
                # 작성일은 진주CCI에서는 목록에 없을 수 있음 (상세페이지에서 확인)
                date = ""
                if len(cells) > 2:
...
# 테스트용 함수
def test_jinjucci_scraper(pages=3):
    """Jinjucci 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedJinjucciScraper()
    output_dir = "output/jinjucci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"Jinjucci 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("Jinjucci 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_jinjucci_scraper(3)