#!/usr/bin/env python3
"""
Enhanced GWRIIA (강원지역산업진흥원) Scraper
Site: https://gw.riia.or.kr/board/notice
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gwriia_scraper.log', encoding='utf-8'),
...
class GWRIIAScraper:
    def __init__(self, base_url="https://gw.riia.or.kr", site_code="gwriia"):
    ...
    def get_page_content(self, url):
        ...
        try:
            ...
            if response.status_code == 500:
                # 500 에러 시 재시도
                logger.warning("500 에러 발생, 2초 후 재시도...")
                time.sleep(2)
                response = self.session.get(url, timeout=30)
                logger.info(f"재시도 응답 상태: {response.status_code}")
    ...
    def get_post_detail(self, post_url):
        ...
        # 기본 내용이 없다면 제목과 메타데이터만 추출
        if not content_element:
            # 제목 추출
            title_element = soup.find('h1') or soup.find('h2') or soup.find('h3')
            if title_element:
                content_html = f"<h1>{title_element.get_text(strip=True)}</h1>"
            else:
                content_html = "<p>내용을 찾을 수 없습니다.</p>"
        else:
    ...
    def save_post_content(self, post, content, attachments):
        ...
        with open(content_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**접수기간:** {post['period']}\n")
            f.write(f"**상태:** {post['status']}\n")
            f.write(f"**작성일:** {post['date']}\n")
...
def main():
    """메인 실행 함수"""
    # 시작 URL
    start_url = "https://gw.riia.or.kr/board/notice"
    
    # 스크래퍼 인스턴스 생성
    scraper = GWRIIAScraper()
