...
class EnhancedJejuCCIScraper(StandardTableScraper):
    """제주상공회의소 전용 스크래퍼 - Enhanced 버전"""
    
    def __init__(self):
        super().__init__()
        
        # 기본 설정
        self.base_url = "https://jejucci.korcham.net"
        self.list_url = "https://jejucci.korcham.net/front/board/boardContentsListPage.do?boardId=10088&menuId=379"
        ...
        logger.info("제주상공회의소 스크래퍼 초기화 완료")
    ...
    def _get_page_announcements(self, page_num: int) -> list:
        ...
        data = {
            ...
            'contId': '',
            'delYn': 'N',
            'menuId': '379',  # 제주상공회의소 공지사항 메뉴 ID
            'boardId': '10088',  # 제주상공회의소 공지사항 게시판 ID
            'readRat': 'A',
            'boardCd': 'N',
            'searchKey': 'A',
            'searchTxt': ''
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for i, row in enumerate(rows):
            try:
                ...
                # contentsView('116108') 패턴 찾기
                id_match = re.search(r"contentsView\('(\d+)'\)", onclick)
                if id_match:
                    content_id = id_match.group(1)
                    detail_url = f"{self.base_url}/front/board/boardContentsView.do?contId={content_id}&boardId=10088&menuId=379"
                else:
                    # 직접 href가 있는 경우
...
def test_jejucci_scraper(pages=3):
    """제주상공회의소 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedJejuCCIScraper()
    output_dir = "output/jejucci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"제주상공회의소 스크래퍼 테스트 시작 - {pages}페이지")
