#!/usr/bin/env python3
"""
Enhanced G-TEXTOPIA (경기섬유종합지원센터) Scraper
...
class GTextopiaScraperScraper:
    ...
    def download_file(self, file_url, attachments_dir, original_filename):
        ...
        try:
            ...
            if not file_url.startswith('http'):
                file_url = urljoin('https://www.gtextopia.or.kr/', file_url)
        ...
        for row in notice_rows:
            try:
                ...
                if 'idx=' in detail_url:
                    idx_match = re.search(r'idx=(\d+)', detail_url)
                    if idx_match:
                        notice_id = idx_match.group(1)
                
                # Extract support organization
                support_org = cells[3].get_text(strip=True)
    ...
    def scrape_notice_detail(self, notice_data):
        ...
        try:
            ...
            if not detail_url.startswith('http'):
                detail_url = urljoin('https://www.gtextopia.or.kr/', detail_url)
            ...
            # Extract content - look for various content containers
            content_html = ""
            
            # Try different possible content containers
            content_selectors = [
                'div.sub-view-content',
                'div.view-content',
                'div.content',
                'div.sub-container'
            ]
            ...
            # If no specific content container found, try to find content area
            if not content_html:
                # Look for any div that might contain the main content
                main_content = soup.find('div', class_='sub-container')
                if main_content:
            ...
            attachment_area = soup.find('div', class_='sub-view-file')
            if attachment_area:
                attachment_links = attachment_area.find_all('a', href=True)
            
            # Method 2: Look for links to files based on common patterns
            if not attachment_links:
                all_links = soup.find_all('a', href=True)
                for link in all_links:
                    href = link.get('href', '')
                    if any(ext in href.lower() for ext in ['.hwp', '.pdf', '.doc', '.docx', '.xls', '.xlsx']):
                        attachment_links.append(link)
            
            # Method 3: Look for download links
            if not attachment_links:
                attachment_links = soup.find_all('a', href=re.compile(r'download|attach|file'))
    ...
    def create_markdown_content(self, notice_data, content_markdown, detail_url):
        ...
        content += f"**지원기관:** {notice_data['support_org']}\n\n"
...
def main():
    # Configuration
    base_url = "https://www.gtextopia.or.kr/corporate/corporate02.html"
    site_code = "gtextopia"
    max_pages = 3
    
    # Initialize scraper
    scraper = GTextopiaScraperScraper(base_url, site_code)
