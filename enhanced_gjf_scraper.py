...
class GJFScraper:
    ...
    def download_file(self, file_url, attachments_dir, original_filename):
        ...
        try:
            ...
            if not file_url.startswith('http'):
                file_url = urljoin('https://www.gjf.or.kr/', file_url)
    ...
    def extract_notice_list(self, soup):
        ...
        if not tbody:
            # Try to find tr elements directly in table
            notice_rows = table.find_all('tr')
            if len(notice_rows) > 1:  # Skip header row
                notice_rows = notice_rows[1:]
            else:
                self.logger.warning("No notice rows found in table")
                return notices
        else:
        ...
        for row in notice_rows:
            try:
                ...
                notice_id = ""
                if 'pst_sn=' in detail_url:
                    id_match = re.search(r'pst_sn=(\d+)', detail_url)
                    if id_match:
                        notice_id = id_match.group(1)
                
                # If detail_url is relative, make it absolute with proper base path
                if detail_url and not detail_url.startswith('http'):
                    detail_url = urljoin('https://www.gjf.or.kr/main/pst/', detail_url)
    ...
    def scrape_notice_detail(self, notice_data):
        ...
        try:
            # Get full detail URL - use the extracted URL directly
            ...
            # Extract content from board view container
            content_html = ""
            
            # Try different possible content containers
            content_selectors = [
                '.board_view_cont',        # Main content container
                '.editor_cont',            # Editor content
                '.hwp_editor_board_content', # HWP editor content
                '.cm_board_view',          # Board view
                '.board_view_cont .editor_cont' # Nested structure
            ]
            ...
            if not content_html:
                ...
                main_divs = soup.find_all('div', class_=re.compile(r'content|editor|view'))
            ...
            # Look for attachment links in file list
            file_list = soup.find('ul', class_='form_file_list')
            if file_list:
            ...
            # Also look for image files in the editor content
            if content_html:
                img_tags = soup.find_all('img')
                for img in img_tags:
                    img_src = img.get('src', '')
                    if img_src and img_src.startswith('/other/editor/process.file.do'):
                        # Extract filename from alt or generate one
                        img_alt = img.get('alt', '')
                        if img_alt:
                            filename = img_alt
                        else:
                            filename = f"image_{len(downloaded_files) + 1}"
                        
                        downloaded_file, file_size = self.download_file(img_src, attachments_dir, filename)
                        if downloaded_file:
                            downloaded_files.append((downloaded_file, file_size))
    ...
    def create_markdown_content(self, notice_data, content_markdown, detail_url):
        ...
        content += f"**구분:** {notice_data['category']}\n\n"
...
def main():
    # Configuration
    base_url = "https://www.gjf.or.kr/main/pst/list.do?pst_id=notice_ap"
    site_code = "gjf"
    max_pages = 3
    
    # Initialize scraper
    scraper = GJFScraper(base_url, site_code)
