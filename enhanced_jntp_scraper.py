#!/usr/bin/env python3
"""
Enhanced JNTP (전남테크노파크) Scraper
Site: https://www.jntp.or.kr/base/board/list?boardManagementNo=16&menuLevel=2&menuNo=60
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('jntp_scraper.log', encoding='utf-8'),
...
class JNTPScraper:
    def __init__(self, base_url="https://www.jntp.or.kr", site_code="jntp"):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                for post in post_data:
                    ...
                    board_no = params.get('boardNo', [''])[0]
                    
                    posts.append({
                        'id': board_no if board_no else str(hash(post['href'])),
                        'number': post['number'],
                        'title': post['title'],
                        'href': post['href'],
                        'author': post['author'],
    ...
    def save_post_content(self, post, content, attachments):
        ...
        with open(content_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**작성자:** {post['author']}\\n")
            f.write(f"**등록일:** {post['date']}\\n")
    ...
    def scrape_page(self, page_url):
        ...
        for post in posts:
            ...
            # 게시글 상세 내용 가져오기
            content, attachments = self.get_post_detail_with_playwright(post['href'])
            if content is None:
                # 내용이 없어도 메타데이터는 저장
...
def main():
    """메인 실행 함수"""
    # 시작 URL
    start_url = "https://www.jntp.or.kr/base/board/list?boardManagementNo=16&menuLevel=2&menuNo=60"
